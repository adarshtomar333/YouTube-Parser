...
def transcribe_audio(title):
    ...
    print(f"\n📝 Transcribing '{title}.wav' (English Only)...")
    model = whisper.load_model("large")  # Change to "small" or "large" for better accuracy
    result = model.transcribe(wav_path, language="en")  # Force transcription in English
...
if __name__ == "__main__":
    ...
    title = download_audio(video_url)  # Step 1: Download Audio
    convert_to_wav(title)              # Step 2: Convert to WAV
    transcribe_audio(title)            # Step 3: Transcribe Audio (English)
